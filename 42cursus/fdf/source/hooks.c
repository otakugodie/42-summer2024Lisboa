...
int	redraw(void *param)
{
	...
	vars = (t_vars *)param;
	isometric_projection(vars->map, vars->height, vars->width,
		&vars->projection);
	render_map(vars);
	return (0);
}

/*Funcion de detección de teclas mediante hook: ESC, '+' o 'z' (zoom in),
//'-' o 'x' (zoom out), Flechas arriba, abajo, izquierda, derecha*/
int	key_hook(int keycode, void *param)
{
	...
	if (keycode == 65307)
		exit(0);
	else if (keycode == 61 || keycode == 43 || keycode == 65451
		|| keycode == 122)
		vars->projection.zoom *= 1.1;
	else if (keycode == 45 || keycode == 95 || keycode == 65453
		|| keycode == 120)
...
//Establece handlers para cerrar ventana (X),
//	detección de teclas y redibujado de la ventana (redraw),
//	permitiendo la interactividad básica del programa FdF*/
